...
md"""
# darwinian-circuit-model-v0.3.jl 

by Rohan Maddamsetti  

Julia version 1.9.  

Tentative title (if anchor point works out): Fitness variance dictates gene expression dynamics in incompatible plasmid systems
...
function gaussian_fitness_function(tetA_copy_number, Tet_conc, σ=SIGMA, r_max=R_MAX)
	...
	left or right based on Tet_conc.
	"""
	fitness = exp(-(Tet_conc - tetA_copy_number)^2 / 2σ^2)
	return fitness
end
...
begin
	...
	for i in 1:num_subpopulations
		cur_time_course = [u[i] for u in final_x]
...
let
	...
	timecourse_plot = plot(final_t, x_1, label="1", xlabel="Time", ylabel="Subpopulation size")
	
	## plot the remaining components over time.
	for i in 2:num_subpopulations
		x_i = subpopulation_time_courses[i]
		plot!(timecourse_plot, final_t, x_i, label=string(i))
	end
	timecourse_plot
end

# ╔═╡ 1462bf95-6e84-453e-bd60-5068cdc48b99
begin ## let's make a matrix saving all the allele frequency dynamics.

	result_matrix = [] ## Initialize an empty matrix

	for i in 1:length(final_x)
		total_N = sum(final_x[i])
		cur_frequency_vec = final_x[i]/total_N
...
time_step_slider = @bind cur_timestep Slider(1:length(final_x), default=1, show_value=true )

# ╔═╡ 965b6b02-2866-4350-adb2-85c38784542f
let
	xvec = collect(1:length(final_x[1]))
	plot(xvec, result_matrix[cur_timestep], label="", xlabel="tetA copy number", ylabel="Current population distribution", ylims=[0,1])
	# Add a vertical dashed line at x = TET_CONC
	vline!([TET_CONC], linestyle=:dash, label="TET_CONC")
end

# ╔═╡ c9608a3b-4ebe-47bf-a91a-59109a8e223b
begin
	tetA_classes = collect(1:length(final_x[1]))
	## get the fitness for each fitness class (defined by tetA copy number).
	growth_rate_vec = fitness_function.(tetA_classes, TET_CONC)
end

# ╔═╡ 50f927a3-92dd-44eb-89cf-41932ac57d3c
result_matrix[cur_timestep]

# ╔═╡ 6b212614-56c8-4543-a40a-dcba46b9072b
let ## make a corresponding plot showing fitness classes rather than tetA copy number.
	scatter(growth_rate_vec, result_matrix[cur_timestep], label="", xlabel="growth rate", ylabel="Current population distribution", ylims=[0,1])
end
...
function odefunc3(du, u, p, t)
	...
	## model [Tet] pulses over time by dividing the current time by 100, and
	## setting [Tet] on if in the first half.
	cur_tet_conc = TET_CONC ##mod(t, 100) in 1:50 ? TET_CONC : 0
...
begin
	...
	## Create an ODEProblem
	prob2 = ODEProblem(odefunc3, initial_pop_vec, tspan)
...
# ╔═╡ db0b18a7-d2b3-4566-a081-12cfa022bafc
final_t2
...
let
	plot(final_t2, tet_pulse_vec, label="Tet pulse regime", xlabel="Time", ylabel="Tet concentration")
end
...
let
	plot(final_t2, mean_fitness_vec, label="Mean fitness", xlabel="Time", ylabel="Mean fitness")
end
...
let
	plot(final_t2, mean_copy_num_vec, label="Mean tetA copy number", xlabel="Time", ylabel="Mean tetA copy number")
end
...
let
	plot(final_t2, fitness_variance_vec, label="Fitness variance", xlabel="Time", ylabel="Fitness variance")
end

# ╔═╡ 2c878d03-cf1f-437b-a50c-a31776fac4c9
let
	plot(final_t2, fitness_variance_vec, label="Fitness variance")
	plot!(final_t2, mean_fitness_vec, label="Mean fitness", xlabel="Time", ylabel="Fitness mean and variance")
end
...
let
	plot(final_t2, fitness_variance_vec, label="Fitness variance")
	plot!(final_t2, d_mean_fitness_vec, label="Rate of change of mean fitness", xlabel="Time", ylabel="Fitness variance and rate of mean fitness change")
end
...
let
	plot(final_t2, copy_num_variance_vec, label="tetA copy number variance", xlabel="Time", ylabel="tetA copy number variance")
end

# ╔═╡ 1ca3d6ae-c75b-4015-8b8b-2f92dd5f9009
let
	plot(final_t2, copy_num_variance_vec, label="tetA copy number variance")
	plot!(final_t2, mean_copy_num_vec, label="mean tetA copy number", xlabel="Time", ylabel="tetA copy number mean and variance")
end
...
let
	plot(final_t2, fitness_variance_vec, label="Fitness variance")
	plot!(final_t2, d_mean_copy_num_vec, label="Rate of change of mean tetA copy number", xlabel="Time", ylabel="tetA copy number derivative and variance")
end

# ╔═╡ e9fec6c8-2275-4c99-933d-3d2d0694c999
let
	plot(final_t2, fitness_variance_vec, label="Fitness variance")
end

# ╔═╡ 58e7619d-265a-4d25-8e66-52a404d533ab
let
	plot(final_t2, d_mean_copy_num_vec, label="Rate of change of mean tetA copy number", xlabel="Time", ylabel="tetA copy number derivative and variance")
end

# ╔═╡ e2db4552-40ae-4f62-b321-08489462dcdd
let
	plot(final_t2, 20*fitness_variance_vec, label="Fitness variance")
	plot!(final_t2, d_mean_copy_num_vec, label="Rate of change of mean tetA copy number", xlabel="Time", ylabel="tetA copy number derivative and variance")
end
