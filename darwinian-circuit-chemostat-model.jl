...
md"""
# darwinian-circuit-chemostat-model.jl 
...
I then modified the exponential growth framework to model continuous culture in a turbidostat, following the framework in "Modelling genetic stability in engineered cell populations" by Ingram and Stan (Nature Comms. 2023).
...
Following the framework in "Modelling genetic stability in engineered cell populations" by Ingram and Stan (Nature Comms. 2023), we use the following dilution function, where $N_{max}$ is the maximum number of cells in the culture:

$dil = \begin{cases}
 \sum_{i} \mathbf{x_i} - N_{max}, & \text{if } \sum_{i} \mathbf{x_i} > N_{max} \\
  0, & \text{otherwise} \\
\end{cases}$ 
...
$\frac{d\mathbf{x}}{dt} = \mathbf{A}\mathbf{x}(t) - dil \cdot \mathbf{x}(t)$.
...
## Define the Dilution function used in Ingram and Stan (2023).
...
let
	...
	plot!(timecourse_plot, legend=:right)
	timecourse_plot
end
